    ...
    public class JoystickForMovement : JoystickHandler
    {
        ...
        private void Update()
        {
            if (_inputVector.x != 0 || _inputVector.y != 0)
            {
                ...
                playerMovement.RotatePlayer(new Vector3(_inputVector.x, 0, _inputVector.y));
            }
            else
            {
                playerMovement.MovePlayer(new Vector3(Input.GetAxis("Horizontal"), 0, Input.GetAxis("Vertical")));
                playerMovement.RotatePlayer(new Vector3(Input.GetAxis("Horizontal"), 0, Input.GetAxis("Vertical")));
            }
        }
    }
}